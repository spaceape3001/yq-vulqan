...
#include <yq/physicsvk/interface/meter/IPosition4M.hpp>
#include <yq/tachyon/api/Tachyon.hpp>

namespace yq::tachyon {

    class AddPosition⁴MCommand;
    class AddPositionʷMCommand;
    ...
    public:
        ...
        Vector4M        position() const override { return m_position; }
        ...
        Meter           position(w_k) const { return m_position.w; }
        
        //!  Set position the position (OVERRIDE THIS TO INTERCEPT/REJECT)
        virtual void    position(set_k, const Vector4M&) override;
        ...
        void            position(add_k, const Vector4M&) override;
        ...
        const Vector4M& position(ref_k) const { return m_position; }

    protected:
        
        Vector4M    m_position = ZERO;

        template <typename C>
        static void init_meta(TachyonMeta::Writer<C>&);
        
        APosition⁴M(const Vector4M& pos=ZERO);
        virtual ~APosition⁴M();
        
        void        position(emit_k);

    private:
        void        on_set_position4(const SetPosition⁴MCommand&);
        void        on_set_positionW(const SetPositionʷMCommand&);
        ...
        void        on_add_position4(const AddPosition⁴MCommand&);
        void        on_add_positionW(const AddPositionʷMCommand&);
