...
namespace yq::tachyon {
    ...
    ACount²U::ACount²U(const Vector2U& cnt) : m_count(cnt)
    {
    }
    ...
    void        ACount²U::count(set_k, const Vector2U& sz) 
    {
        m_count  =    sz;
        mark();
        count(EMIT);
    ...
    void        ACount²U::count(add_k, const Vector2U&v) 
    {
    ...
    void        ACount²U::count(multiply_k, const Vector2U& v) 
    {
        count(SET, Vector2U( m_count.x*v.x, m_count.y*v.y ));
    }
    ...
    void        ACount²U::on_set_count2(const SetCount²UCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        count(SET, cmd.count());
    ...
    void        ACount²U::on_add_count2(const AddCount²UCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        count(ADD, cmd.Δ());
    ...
    void        ACount²U::on_multiply_count2(const MultiplyCount²UCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        count(MULTIPLY, cmd.δ());
