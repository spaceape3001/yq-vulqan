...
namespace yq::tachyon {
    ...
    void        ACount⁴::on_set_count4(const SetCount⁴UCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        count(SET, cmd.count());
    ...
    void        ACount⁴::on_set_countW(const SetCountʷUCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        count(SET, W, cmd.w());
    ...
    void        ACount⁴::on_add_count4(const AddCount⁴UCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        count(ADD, cmd.Δ());
    ...
    void        ACount⁴::on_add_countW(const AddCountʷUCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        count(ADD, W, cmd.Δw());
    ...
    void        ACount⁴::on_multiply_count4(const MultiplyCount⁴UCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        count(MULTIPLY, cmd.δ());
    ...
    void        ACount⁴::on_multiply_countW(const MultiplyCountʷUCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
