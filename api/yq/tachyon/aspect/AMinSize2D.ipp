...
namespace yq::tachyon {
    ...
    void        AMinSize²D::min_size(set_k, const Size2D& sz) 
    {
        m_min_size  =    sz;
        mark();
        min_size(EMIT);
    ...
    void        AMinSize²D::min_size(add_k, const Size2D&v) 
    {
    ...
    void        AMinSize²D::min_size(multiply_k, const Vector2D& v) 
    {
        min_size(SET, Size2D( m_min_size.x*v.x, m_min_size.y*v.y ));
    }
    ...
    void        AMinSize²D::on_set_min_size2(const SetMinSize²DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
    ...
    void        AMinSize²D::on_add_min_size2(const AddMinSize²DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        min_size(ADD, cmd.Δ());
    ...
    void        AMinSize²D::on_multiply_min_size2(const MultiplyMinSize²DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        min_size(MULTIPLY, cmd.δ());
