...
namespace yq::tachyon {
    ...
    void        AMinSize³D::min_size(set_k, const Size3D& sz) 
    {
        m_min_size  =    sz;
        mark();
        min_size(EMIT);
    ...
    void        AMinSize³D::min_size(add_k, const Size3D&v) 
    {
    ...
    void        AMinSize³D::min_size(multiply_k, const Vector3D& v) 
    {
        min_size(SET, Size3D( m_min_size.x*v.x, m_min_size.y*v.y, m_min_size.z*v.z ));
    }
    ...
    void        AMinSize³D::on_set_min_size3(const SetMinSize³DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
    ...
    void        AMinSize³D::on_add_min_size3(const AddMinSize³DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        min_size(ADD, cmd.Δ());
    ...
    void        AMinSize³D::on_multiply_min_size3(const MultiplyMinSize³DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        min_size(MULTIPLY, cmd.δ());
