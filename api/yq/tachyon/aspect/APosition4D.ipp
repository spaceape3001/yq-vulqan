...
#include "APosition4DWriter.hxx"

#include <yq/tachyon/aspect/APosition4D.hpp>
...
namespace yq::tachyon {
    APosition⁴D::APosition⁴D(const Vector4D& pos) 
    {
        if(!is_nan(pos))
            m_position = pos;
    }
    ...
    void        APosition⁴D::position(set_k, const Vector4D& sz) 
    {
        if(is_nan(sz))
            return;
    ...
    void        APosition⁴D::position(set_k, w_k, double v) 
    {
        position(SET, Vector4D(m_position.x, m_position.y, m_position.z, v ));
    }

    void        APosition⁴D::position(add_k, const Vector4D&v) 
    {
    ...
    void        APosition⁴D::position(add_k, w_k, double v) 
    {
    ...
    void        APosition⁴D::position(multiply_k, const Vector4D& v) 
    {
        position(SET, Vector4D( m_position.x*v.x, m_position.y*v.y, m_position.z*v.z, m_position.w*v.w ));
    }
    ...
    void        APosition⁴D::position(multiply_k, w_k, double v)
    {
        position(SET, Vector4D( m_position.x, m_position.y, m_position.z, m_position.w*v ));
    }

    void        APosition⁴D::on_set_position4(const SetPosition⁴DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
    ...
    void        APosition⁴D::on_set_positionW(const SetPositionʷDCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
    ...
    void        APosition⁴D::on_add_position4(const AddPosition⁴DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        position(ADD, cmd.Δ());
    ...
    void        APosition⁴D::on_multiply_position4(const MultiplyPosition⁴DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        position(MULTIPLY, cmd.δ());
    ...
    void        APosition⁴D::on_multiply_positionW(const MultiplyPositionʷDCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
