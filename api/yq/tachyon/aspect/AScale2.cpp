...
namespace yq::tachyon {
    ...
    void        AScale²::on_set_scale2(const SetScale²DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        scale(SET, cmd.scale());
    ...
    void        AScale²::on_add_scale2(const AddScale²DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        scale(ADD, cmd.Δ());
    ...
    void        AScale²::on_multiply_scale2(const MultiplyScale²DCommand&cmd)
    {
        if(cmd.target() != typed())
            return;
        scale(MULTIPLY, cmd.δ());
