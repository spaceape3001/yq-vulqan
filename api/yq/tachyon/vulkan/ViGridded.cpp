...
#include <yq/tachyon/vulkan/ViGridded.hpp>
...
namespace yq::tachyon {
    namespace errors {
        using gridded_bad_state                   = error_db::entry<"Gridded is in a bad state">;
        using gridded_bad_format                  = error_db::entry<"Gridded format is unknown">;
        using gridded_existing                    = error_db::entry<"Gridded already created">;
        using gridded_cant_create_temporary       = error_db::entry<"Unable to create temporary image">;
        using gridded_empty                       = error_db::entry<"Gridded is empty">;
        using gridded_incompatible_images         = error_db::entry<"Griddeds are incompatible to be layered">;
        using gridded_view_cant_create            = error_db::entry<"Unable to create image view">;
    }
    
    Expect<GriddedDataPtr>   export_gridded(ViDevice& viz, VkImage img, const ViGriddedExport& vix)
    {
        if(!img)
        ...
        if(!cnt)
            return errors::gridded_empty();

        GriddedDataInfo  info;
        info.type       = (GriddedDataType) vix.type;
        ...
        if(vix.desired && (vix.desired != vix.format)){
            ...
            temp    = new ViGridded(viz, info, p4);
            if(!temp->valid())
                return errors::gridded_cant_create_temporary();
        }
        
        size_t       bpp       = ViGridded::format_bytes((VkFormat) info.format.value());
        if(!bpp)
            return errors::gridded_bad_format();
        ...
        ec  = viz.queue_task(viz.graphics_queue(), downloadTask);
    ...
    size_t       ViGridded::format_bytes(VkFormat fmt)
    {
        switch(fmt){
    ...
    ViGridded::ViGridded(ViDevice&viz, const GriddedData&img, const Param& p)
    {
        if(viz.device()){
    ...
    ViGridded::ViGridded(ViDevice&viz, const GriddedDataInfo& info, const Param& p)
    {
        if(viz.device()){
    ...
    ViGridded::ViGridded(ViDevice&viz, std::span<const GriddedDataCPtr> images, const Param& p)
    {
        if(viz.device()){
    ...
    std::error_code ViGridded::_init(ViDevice&viz, const GriddedDataInfo& info, const Param& p)
    {
        m_device                = &viz;
        m_info                  = info;
    ...
    void            ViGridded::barrier(VkCommandBuffer cmd, const Respec& spec)
    {
        VqImageMemoryBarrier imb;
    ...
    std::error_code ViGridded::_init(ViDevice& viz, const GriddedData& img, const Param& p)
    {
        ...
        if(ec != std::error_code()){
        vizWarning << "ViGridded: Unable to initialize: " << ec.message();
            return ec;
        }
        ...
        ec = viz.queue_task(viz.graphics_queue(), uploadTask);
        if(ec != std::error_code())
            return ec;
    ...
    std::error_code ViGridded::_init(ViDevice&viz, const std::span<const GriddedDataCPtr>&imgs, const Param& p)
    {
        //  check for nulls
        for(const GriddedDataCPtr& img : imgs){
            if(!img){
                vizWarning << "ViGridded: null image submitted";
                return errors::null_pointer();
            }
        }
    
        // check for compatible characteristics
        GriddedDataInfo      info = imgs[0]->info;
        //bool            diffSize    = false;
        for(const GriddedDataCPtr& img : imgs){
            ...
            if(info.type != img->info.type){
                vizWarning << "ViGridded: Image type mismatch";
                return errors::gridded_incompatible_images();
            }
            if(img->info.arrayLayers != 1){
                vizWarning << "ViGridded: This is already a layered image";
                return errors::gridded_incompatible_images();
            }
        }
        ...
        //  Load the images
        std::vector<ViGriddedPtr>    images;
        for(const GriddedDataCPtr& img : imgs){
            Param p3;
            ...
            ViGriddedPtr  local = new ViGridded(viz, *img, p3);
            if(!local->valid())
                return errors::insufficient_gpu_memory();
        ...
        auto compositeTask = [&](VkCommandBuffer cmd){
            ...
            for(size_t n=0;n<images.size();++n){
                const GriddedData&   ras = *(imgs[n]);
                const ViGridded&  img = *images[n];
                /*
        ...
        ec = viz.queue_task(viz.graphics_queue(), compositeTask);
        if(ec != std::error_code())
            return ec;
        return {};
    }

    void ViGridded::_kill()
    {
    ...
    std::error_code ViGridded::init(ViDevice&viz, const GriddedData&img, const Param&p)
    {
        if(m_device){
            if(!consistent())
                return errors::gridded_bad_state();
            return errors::gridded_existing();
        }
    ...
    bool            ViGridded::consistent() const
    {
        return m_device ? (m_image && m_allocation) :
            (!m_image && !m_allocation);
    }
    
    bool            ViGridded::valid() const
    {
        return m_device && m_image && m_allocation;
    }
